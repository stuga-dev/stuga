 ...
 * flush that turns pending edits into a snapshot and an index job.
 *
...
import { hashText, pruneUnretained, VersionRing, type RingState } from "./retention.js";
...
export class DocStore {
  ...
  /** The in-flight flush; a head install waits for it so the two never claim the same seq. */
  ...
  private journal(update: Uint8Array, origin: unknown): void {
    ...
    if (origin && typeof origin === "object") {
      ...
      if (o.agent) this.contributors.add(o.agent);
      else if (o.alias) this.contributors.add(o.alias);
    }
  }
  ...
  /**
   * Arm the flush backstop while dirty. An earlier alarm is left alone. Without a
   * docId flush cannot run, so an alarm would re-arm forever: clear it instead.
   */
  async armAlarms(): Promise<void> {
    ...
    const due = Date.now() + DOC_FLUSH_INTERVAL_MS;
  ...
  async flush(reason: string): Promise<void> {
    ...
    // Everything a failed flush must roll back, so the actor stays dirty and the alarm retries.
    const prev = {
      seq: this.seq,
      dirty: this.dirty,
    ...
    try {
      ...
      // Everything held is parked behind a missing dependency, so the document
      // decodes empty: snapshot it, but indexing would blank its search text.
      const pendingOnly = plain.trim() === "" && this.parkedBytes() > 0;
      if (pendingOnly) {
        ...
        const now = Date.now();
        const contentHash = this.ring.due(nextSeq, reason, now) ? hashText(plain) : null;
        recordedVersion = contentHash !== null && this.ring.changedSince(contentHash);
        if (recordedVersion) evictedVersion = this.ring.note(nextSeq, now, contentHash!);
        ...
        await this.env.jobs.send({
          ...
          ...(recordedVersion
            ? { recordVersion: true as const, versionFloor: this.ring.floor }
      ...
      this.updatesSincePersist = 0;
      if (this.pending === flushedPending) {
        this.pending = null;
        this.dirty = false;
  ...
  /**
   * Run `fn` with flushes held off and any in-flight flush drained first, so a
   * head install and a flush never claim the same seq.
   */
  ...
  /** Forget the in-memory document so the next request re-hydrates from storage. */
  unload(): void {
    this.doc = new Y.Doc();
    this.bindDoc();
    this.loaded = false;
