/**
 * The `collections` tool on /mcp: an agent manages its person's collections through the same
 * operations as REST, a read-only key only reads them, and a collection scope on `retrieve` and
 * `docs` search is exactly the collection's documents.
 */
...
function agentCtx(scope?: { folders: string[] | null; readOnly: boolean }): Ctx {
  return {
    ...
    workspaceId: "ws1",
    role: "member",
    ...(scope ? { scope: { ...scope, keyId: "agent-1" } } : {}),
    env: {
      jobs: { send: jobsSend },
...
describe("the collections tool", () => {
  it("creates a collection for the person the connector acts for", async () => {
    const r = await callTool(agentCtx(), "collections", { action: "create", name: "Launch notes" });
  ...
  it("lists, opens, renames and deletes the person's collections", async () => {
    ...
    expect(JSON.parse((await callTool(agentCtx(), "collections", { action: "rename", collection_id: "col_1", name: "Renamed" })).text)).toEqual({
      collection_id: "col_1",
      name: "Renamed",
    });
    expect(JSON.parse((await callTool(agentCtx(), "collections", { action: "delete", collection_id: "col_1" })).text)).toEqual({ deleted: true });
  ...
  it("adds only what a folder-scoped key can read, and tells the agent what it skipped", async () => {
    ...
    const r = await callTool(agentCtx({ folders: ["f1"], readOnly: false }), "collections", {
      action: "add_items",
      collection_id: "col_1",
  ...
  it("removes items", async () => {
    const r = await callTool(agentCtx(), "collections", { action: "remove_items", collection_id: "col_1", folder_ids: ["f1"] });
  ...
  it.each([
    ...
    expect(await callTool(agentCtx({ folders: ["f1"], readOnly: false }), "collections", args)).toEqual({ isError: true, text: "error: collection not found" });
  ...
  it("answers someone else's collection as not found", async () => {
    ...
    expect(await callTool(agentCtx(), "collections", { action: "delete", collection_id: "col_1" })).toEqual({ isError: true, text: "error: collection not found" });
    expect(vi.mocked(db.deleteCollection)).not.toHaveBeenCalled();
  });

  it("asks for the arguments an action needs before calling the node", async () => {
    expect(await callTool(agentCtx(), "collections", { action: "create" })).toEqual({ isError: true, text: "error: create requires `name`" });
    expect(await callTool(agentCtx(), "collections", { action: "rename", name: "x" })).toEqual({ isError: true, text: "error: rename requires `collection_id`" });
    expect(await callTool(agentCtx(), "collections", { action: "add_items", collection_id: "col_1" })).toEqual({
      isError: true,
  ...
  it("audits the change and then the answered tool call, both as the agent acting for its person", async () => {
    await callTool(agentCtx(), "collections", { action: "rename", collection_id: "col_1", name: "Renamed" });
    const rows = audits();
    expect(rows.map((r) => r.action)).toEqual(["collection.rename", "mcp.collections.rename"]);
...
describe("a read-only key", () => {
  ...
  ])("is refused $action, and the refusal is audited", async (args) => {
    expect(await callTool(readOnly(), "collections", args)).toEqual({ isError: true, text: `error: ${READ_ONLY_MESSAGE}` });
    expect(audits()).toEqual([expect.objectContaining({ action: `mcp.collections.${args.action}`, status: "denied" })]);
    expect(vi.mocked(db.createCollection)).not.toHaveBeenCalled();
    expect(vi.mocked(db.renameCollection)).not.toHaveBeenCalled();
    expect(vi.mocked(db.deleteCollection)).not.toHaveBeenCalled();
...
describe("a collection scope on /mcp", () => {
  it("searches exactly the collection's documents", async () => {
    await callTool(agentCtx(), "docs", { action: "search", q: "launch", collection_id: "col_1" });
  ...
  it("never falls back to every document for a collection it cannot use", async () => {
    ...
    expect(await callTool(agentCtx(), "docs", { action: "search", q: "launch", collection_id: "col_gone" })).toEqual({
      isError: true,
