...
| `POST /api/workspaces/import?name=&default_doc_access=` | A new workspace from a [workspace archive](workspace-archive.md), the `.stuga.zip` as the body, within the node's upload limit. The caller owns it and everything in it, and `name` defaults to the archive's. The whole archive is checked first: `400` names what is wrong with it, and nothing is made. A write that fails after that deletes the workspace again and answers `500`, and a node stopped partway deletes it when it starts again. No list shows the workspace until the import is done. Answers `201` as `POST /api/workspaces` does, once the import is done, with `start_doc_id` when the archive names a document to open first. A person imports one archive at a time, and a node three: `409` while the caller's own import, or three others, are under way, and while a backup waits for the imports and exports under way. An import still writing after 50 minutes stops, before the web app gives up waiting at an hour: it deletes the workspace and answers `500`. An import whose client goes away goes on. It works before the caller has a workspace. |
| `GET /api/workspace-samples` | The [sample workspaces](workspace-archive.md#the-samples-index) a new workspace can start from, in the list's order: `{ samples: [{ id, title, description, name, langs }] }`, or `{ samples: [], unavailable: true }` when the node has no list from [`SAMPLES_URL`](configuration.md#network), or its last look for one failed. It works before the caller has a workspace. |
| `POST /api/workspaces` with `sample` | `{ sample, name?, default_doc_access? }`: a new workspace from the sample whose `id` is `sample`. The node downloads its archive, checks it against the list's size and SHA-256 and as `POST /api/workspaces/import` does, and imports it; `name` defaults to the sample's. Then Sample agent, an agent with no key acting for the caller, proposes the sample's changes and posts its comments, each change waiting for the caller's review. `400` for an id the list does not have; `502` `could not download the sample` when the list or the archive cannot be had, or the archive does not pass the checks, and nothing is made; `500` when a write or one of Sample agent's steps fails, after deleting the workspace again. Answers `201` as the import does, with `start_doc_id` when the sample names a document to open first, and counts as one of the caller's and the node's imports (`409`). Without `sample`, it makes an empty workspace. |
| `GET /api/workspaces/:id/export` | The workspace as a [workspace archive](workspace-archive.md), `<name>.stuga.zip`, streamed: every folder, document and database the caller can open, with its comments and images. A workspace owner or admin, with `:id` the workspace the `x-stuga-workspace` header names. `413`, before anything is sent, when the caller can open more items, row pages, rows or files than an archive holds, folders nested more than 32 deep, a document with more comments or a longer body, or documents that together unpack to more. An archive that would pass another cap, such as its size, stops partway through the download. A person exports one workspace at a time, and a node three: `409` while the caller's own export, or three others, are under way, and while a backup waits for the imports and exports under way. An export also stops partway when the client reads nothing for a minute, when it is not done after 50 minutes, and when the caller is no longer an owner or admin; what each item holds is what the caller can open as it is written. |
...
| Route | |
|---|---|
| `GET /auth/config` | No credential. `{ provider, unclaimed, node_name, node_label, origin, branding, search_languages }`: `provider` is `{ label }` when the node has an identity provider, else null, `unclaimed` is true until the first account exists, `node_name` is the name an administrator gave the node, null until there is one, `node_label` is what tells the node apart (the name, else its host), `origin` is `PUBLIC_ORIGIN`, `branding` is `{ accent_color }`, and `search_languages` is, while `unclaimed`, the [search languages](configuration.md#search-languages) the node took from `SEARCH_LANGUAGES` or its search indexes, else null. |
| `POST /auth/register` | `{ username, password, name?, invite?, setup_code?, update_check?, time_zone?, search_languages? }` → `201` with a token pair. The first account needs the node's `setup_code` (`403` `setup_code_required`, `setup_code_invalid`); case, spaces and dashes in it do not matter. Every account after the first needs an invite, and a `setup_code` it carries is ignored. A reserved or taken username is `409` (`username_reserved`, `username_taken`) with a free `suggestion`. `update_check: false` from the account that claims the node turns the look for a newer version off before the node has made one; from any later account it changes nothing. `time_zone`, an IANA name, becomes the node's from the account that claims it; a name the node does not know is ignored. `search_languages`, a list of `ko` and `ar` (anything else is `400`), become the node's [search languages](configuration.md#search-languages) from the account that claims it, and change nothing from a later one. |
